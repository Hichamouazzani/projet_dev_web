...
<form class="form-control" on:submit|preventDefault={handleCreatePost}>
  <label for="post" class="label">
    <span class="label-text text-xl">What would you like to OTwit?</span>
  </label> 
  ...
  {#await createPostPromise}
    <button disabled type="button" class="btn">OTwit that Shwit!</button> 
  {:then {data, error}}
    <button class="btn">OTwit that Shwit!</button>
    {#if data}
